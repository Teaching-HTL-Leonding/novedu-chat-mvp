...
// an inline discussion. The signed link params travel with every action so the
// server re-verifies the link each time. NOTHING is stored about the run — the
// summary is client-only and a reload restarts the quiz.

export interface QuizLinkParams {
  quiz: string;
  start: string;
  end: string;
  sig: string;
...
export function QuizRunner({ quiz, link }: { quiz: QuizPublic; link: QuizLinkParams }) {
  ...
  async function handleSubmit(question: QuizQuestionPublic) {
    ...
    setGrading(true);
    setError(null);
    const res = await submitAnswer({ ...link, questionId: question.id, answer: trimmed });
    setGrading(false);
    if (!res.ok) {
  ...
  const discussionHeaders = discussion
    ? {
        "x-quiz-url": link.quiz,
        "x-quiz-start": link.start,
        "x-quiz-end": link.end,
        "x-quiz-sig": link.sig,
        "x-thread-token": discussion.threadToken,
      }
    : null;
