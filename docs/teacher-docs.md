...
hand-maintained Markdown — and the **site** (`teacher-docs-site/`) — an Astro Starlight
...
> TL;DR: to change a chapter, edit it in `teacher-docs/content/` — by hand or via
...
The whole corpus is **human-owned**: `content/` is the source of truth, edited
...
- `src/content.config.ts` defines the `docs` collection with Starlight's
  ...
  and editing a chapter live-reloads a running dev server. (A plain glob
