...
- A recipe with **behavior** is a component, not a class string: `DialogShell` (`components/ui/dialog-shell.tsx`) owns every modal's open/close/Escape/backdrop contract and its flex-column shell — height is its cva `size` variant (`tall` default / `fit` shrink-to-fit; never a raw `h-*` delta, see the variant's comment), width a `className` delta; `PageBody` (`components/page-main.tsx`) owns the page gutter + scroll rhythm — and the page **canvas**: a full-bleed gray (`slate-100`) scroller (the shared `PAGE_CANVAS` constant — negative margins undo `Main`'s centered max-width, so the page scrollbar sits at the window edge) with the content re-centered inside; content surfaces on it (tables, dashboard cards, editors, the writing chat pane) carry `bg-card`/`bg-background`. The viewport-bounded writing split composes `PAGE_CANVAS` without the scroller. **Page width**: every surface is bounded at `max-w-7xl` (1280px); the sole exception is `PageBody`'s `wide` prop, which the list pages use for a `fit-content` column clamped to 1280–1760px and sized by the table (`docs/filtered-lists.md`). The status bar is the dark counterpart (`slate-900`, white ink) and is rendered by the root layout through `AppChrome` (`components/app-chrome.tsx`), which drops it on the two bare routes — `/sign-in` and `/device`, both self-contained screens that name the app and the account in their own body text; directly below it, outside `AppChrome` and so on every route, the root layout renders the **environment ribbon** (`components/environment-ribbon.tsx`, a client component): it maps the browser's `location.hostname` to LOCAL (violet) / DEV (amber) / PROD (emerald), renders nothing on the server or on any other host, and its "X" hides it for the tab's session via `sessionStorage` (the teacher guide shows the same colours for LOCAL and DEV only — `docs/teacher-docs.md`); the tutor/quiz-discussion chat surfaces don't use `PageBody` and stay on the white body. Pure-look recipes shared across a couple of surfaces ship as exported class constants composed via `cn()` deltas (`DIALOG_BODY`, `META_LABEL`, `MENU_PANEL`/`MENU_ITEM`, the `CENTERED_CARD*` trio on `Notice`, `SEGMENTED_NAV` + its `segmentedItemVariants` for the chart pages' time filters).
