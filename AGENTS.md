...
- The **`coding`** module's **`POST /api/coding/v1/chat/completions`** is the second public, non-Entra route (also in the `proxy.ts` matcher): an external coding agent has no session, so it authenticates with the **code as the bearer API key** (`checkCode` re-verified every request — never a bare lookup). It has **no** in-app chat and **no** `x-thread-token`. It is **always anonymous** (the API path carries no `oid`), and the teacher's system prompt + the pinned model stay **server-side** (the proxy injects the prompt and pins the model; neither reaches the browser).
- **LLM connectivity is server-only and isolated** behind `lib/llm/` (`docs/ai-models.md`): the SCCH/Azure-Foundry distinction exists ONLY in `resolveLanguageModel` (agents), `resolveChatEndpoint` (the coding route, side-effect-free, incl. its `adaptBody` dialect hook), and `providerUnavailableReason` (`lib/llm/availability.ts`, app-only — never in the CLI bundle); endpoints, the SCCH key, and Entra tokens never reach the browser. Azure Foundry auth is **passwordless Entra** — `buildCognitiveServicesCredential()` (`az login` → Managed Identity, role `Cognitive Services OpenAI User`), never `DefaultAzureCredential`, never an API key; tokens auto-refresh via one `getBearerTokenProvider` singleton and acquisition is bounded (15 s) so no request can hang on it. The activity YAML's `llm.provider` (missing ⇒ `SCCH`) is the only selector.
...
Read before touching: `app/api/coding/**`, `app/[code]/render-coding.tsx`, `app/[code]/_coding/**`, `lib/coding-*.ts`, `lib/llm/endpoint.ts`, `lib/code-modules/coding.ts`, the `api/coding` matcher in `proxy.ts`, `activities/coding/` (sample YAML).

- The `coding` module: an **OpenAI-compatible Chat Completions endpoint** an external coding agent (e.g. little-coder) points at, so students code against the activity's model (SCCH or Azure Foundry) with a teacher-authored system prompt. It adds itself through the codes seams (descriptor **without a `runtime`** + registry line + label + student render case + a `coding` validator/`readAnonymousFlag` branch) — the generic flow is untouched.
- The endpoint is a **thin pass-through proxy** (`app/api/coding/v1/chat/completions`, server-only, no Mastra): `checkCode` (bearer key, verbatim — no `sk-` stripping) → `loadCoding` → **append** the teacher's system prompt to the end of the client's system message (teacher gets the final word) + **pin** the model (`buildUpstreamChatBody`, `lib/coding-proxy.ts`) + the provider's `adaptBody` **dialect hook** (Foundry: `max_tokens` → `max_completion_tokens`, `temperature`/`top_p` dropped; SCCH: identity) → forward to `resolveChatEndpoint(provider)`'s URL with `signal: req.signal` (`lib/llm/endpoint.ts`, side-effect-free — does NOT import `app/mastra/scch.ts`) → **pipe the response stream back unparsed**, so client-side tools + streaming are preserved. Oversized bodies are rejected (`413`). See the security block for the access/anonymity model.
...
- Authoring **validation is a strict `CodingYamlSchema` gate** (`lib/coding-schema.ts` source of truth, run by `loadAndCheckCoding` in `lib/coding-validate.ts`) that blocks an invalid save and rejects fields coding lacks (incl. `anonymous` — the seam freezes `anonymous: true` itself); `activities/coding/coding-yaml.schema.json` is the editor-IntelliSense mirror. The lenient runtime read is `parseCoding` (`lib/coding-yaml.ts`). Model **allowlist + `/v1/models`**, **rate limiting**, and **usage metrics** are deferred.

### AI models & LLM providers → `docs/ai-models.md`

Read before touching: `lib/llm/**`, `app/mastra/scch.ts`, `lib/scch-endpoint.ts`, `buildCognitiveServicesCredential` in `lib/azure-credential.ts`, the `llm:` block of any activity schema, the Foundry probe in `lib/health.ts`.

- Two OpenAI-compatible providers: **SCCH** (self-hosted vLLM, static key) and **Azure Foundry** (Azure OpenAI's `/openai/v1` surface — no `api-version`, the deployment name is the `model`, Entra bearer auto-refreshed by one `getBearerTokenProvider` singleton, scope `cognitiveservices.azure.com/.default`). Every activity YAML selects with `llm.provider` (missing ⇒ SCCH); Foundry is optional — without `AZURE_FOUNDRY_ENDPOINT` the app runs SCCH-only (lazy construction, `foundryConfigured()` gates the `/health` rows).
- The provider branch exists in exactly THREE functions: `resolveLanguageModel` (`lib/llm/model.ts`, agents — may import `app/mastra/scch.ts`), the side-effect-free `resolveChatEndpoint` incl. `adaptBody` (`lib/llm/endpoint.ts`, the coding route — must NOT), and `providerUnavailableReason` (`lib/llm/availability.ts` — the save-time + runtime gate for a provider this server has not configured; app-only, never in the CLI bundle). The `createOpenAI` **names** (`scch`/`azure-foundry`, `lib/llm/provider.ts`) are the metering contract: the usage exporter maps MODEL_GENERATION span `attributes.provider`/`attributes.model` back via `providerFromModelProviderId` — renaming breaks attribution.
- Adding a provider = one branch in each of the three functions + a name constant/mapping + the schema enum literal. Live Foundry tests are `@live-llm` (no Managed Identity in CI).
...
- The teacher-only read surface at `/usage` over `novedu_usage_by_code`: a stacked token-over-time **bar chart** + data table, three **pies** (per module, per code top-9 + Other, per **model** top-9 + Other — model ids are provider-specific, so the model pie doubles as the SCCH vs. Azure Foundry split), and two windowed **KPIs** (chats, quiz answers graded). One **time filter** (`24h/7d/30d/365d`, all **UTC**) governs every panel including the KPIs.
