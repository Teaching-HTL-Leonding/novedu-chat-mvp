...
Read before touching: `app/api/coding/**`, `app/[code]/render-coding.tsx`, `app/[code]/_coding/**`, `lib/coding-*.ts`, `lib/scch-endpoint.ts`, `lib/code-modules/coding.ts`, the `api/coding` matcher in `proxy.ts`, `activities/coding/` (sample YAML).
