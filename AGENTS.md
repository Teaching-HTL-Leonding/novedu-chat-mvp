...
Read before touching: `lib/writing-*.ts`, `app/[code]/_writing/**`, `app/[code]/render-writing.tsx`, `app/mastra/writing-agents.ts`, `lib/code-modules/writing.ts`, `novedu_writing_submissions`.

- The `writing` module: a student writes Markdown in the `/files` CodeMirror editor (Markdown mode) on a split screen, an AI assistant gives feedback, and the student **saves** their text. It adds itself through the codes seams (descriptor + registry line + label + render case + a `writing` validator/`readAnonymousFlag` branch + the agent + the store) — the generic flow is untouched.
