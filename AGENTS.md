...
- Authoring **validation is a strict `CodingYamlSchema` gate** (`lib/coding-schema.ts` source of truth, run by `loadAndCheckCoding` in `lib/coding-validate.ts`) that blocks an invalid save and rejects fields coding lacks (incl. `anonymous` — the seam freezes `anonymous: true` itself); `coding/coding-yaml.schema.json` is the editor-IntelliSense mirror. The lenient runtime read is `parseCoding` (`lib/coding-yaml.ts`). Model **allowlist + `/v1/models`**, **rate limiting**, and **usage metrics** are deferred.
