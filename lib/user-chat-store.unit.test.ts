...
beforeEach(() => {
  ...
  // Default tutor: anonymous (the YAML default) — nothing must be stored.
  mocks.loadAndBuildTutorPrompt.mockResolvedValue({ ok: true, anonymous: true });
});

describe("recordUserChat — anonymous gate", () => {
  it("stores NOTHING for an anonymous tutor (the default)", async () => {
    await recordUserChat(CODE, THREAD, USER, TUTOR_URL);
  ...
  it("does NOT cache a failed YAML load — attribution recovers on the next call", async () => {
    ...
    await recordUserChat(CODE, THREAD, USER, TUTOR_URL); // transient failure, no row
    mocks.loadAndBuildTutorPrompt.mockResolvedValue({ ok: true, anonymous: false });
    await recordUserChat(CODE, THREAD, USER, TUTOR_URL); // host recovered
  ...
  it("does NOT cache a throwing YAML load either", async () => {
    ...
    await recordUserChat(CODE, THREAD, USER, TUTOR_URL); // never throws
    mocks.loadAndBuildTutorPrompt.mockResolvedValue({ ok: true, anonymous: false });
    await recordUserChat(CODE, THREAD, USER, TUTOR_URL);
    expect(mocks.insertValues).toHaveBeenCalledTimes(1);
  });
...
describe("recordUserChat — dedupe & robustness", () => {
  ...
  it("rejects malformed thread ids without touching YAML or database", async () => {
    await recordUserChat(CODE, "../../etc/passwd", USER, TUTOR_URL);
  ...
  it("retries after a transient database error (decision not cached)", async () => {
    ...
    await recordUserChat(CODE, THREAD, USER, TUTOR_URL); // fails, never throws
    await recordUserChat(CODE, THREAD, USER, TUTOR_URL); // retried
