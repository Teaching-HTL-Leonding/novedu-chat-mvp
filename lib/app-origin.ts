...
// point back at this deployment — the chat link for a tutor code, and the public
// GET URL of an app-hosted YAML file.
//
// Prefer the explicit TUTOR_CODE_ORIGIN env var (set it in production — forwarded
// headers are only as trustworthy as the proxy chain in front of the app).
// Without it, fall back to the request's forwarded/host headers, which is fine
// for local dev. Multi-hop proxies append comma-separated values, so only the
// first (client-most) entry counts. Throws if no host can be determined.
