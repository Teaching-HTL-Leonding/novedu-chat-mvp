...
//
// SQL Server's default collation is case-insensitive, so a plain `LIKE` already
// matches regardless of case — no `LOWER()` needed. We DO escape the LIKE
// wildcards (`%`, `_`, `[`) in the user's term so a search for "50%" looks for a
// literal "50%", not "starts with 50". The escape character is a backslash,
// declared per-column with `ESCAPE '\'`.

const LIKE_ESCAPE_CHAR = "\\";

// Escape the LIKE metacharacters in a raw user term. Backslash MUST be escaped
// first (it is the escape char itself), then the wildcards. `]` needs no
// escaping — only `[` opens a character class in T-SQL LIKE. Exported for unit
// tests.
 ...
 * `col1 LIKE %term% OR col2 LIKE %term% …`, with LIKE wildcards in `term`
...
export function containsAny(term: string, columns: AnyColumn[]): SQL | undefined {
  ...
  return or(...columns.map((column) => sql`${column} LIKE ${pattern} ESCAPE ${LIKE_ESCAPE_CHAR}`));
}
