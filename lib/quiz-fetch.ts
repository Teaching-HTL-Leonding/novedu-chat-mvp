...
// `loadAppHostedYaml`, and renders the quiz-level `instructions` host text (with any
...
// chat. The single definition shared by the `/q` page, `submitAnswer`,
  ...
  return loadAppHostedYaml(quizUrl, parseQuiz, "quiz", async (parsed, { url, fetcher }) => {
    ...
    return {
      ...
      quiz: {
        ...
        instructionsPreamble: resolved.prompt.trimEnd(),
      },
    };
  });
}
