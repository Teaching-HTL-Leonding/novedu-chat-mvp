...
vi.mock("@/lib/tutor-code-actions", () => ({
  ...
  updateTutorCodeAction: vi.fn(async (code: string, _prev: unknown, formData: FormData) => {
    updated.push({ code, formData });
    return updateResult;
  }),
  deleteTutorCodeAction: vi.fn(async () => ({ ok: true })),
}));
...
import { TutorCodeForm } from "@/app/tutor-codes/tutor-code-form";
...
test("create: submits tutor URL, note, and window as unix seconds (local-time converted)", async () => {
  created.length = 0;
  const screen = await render(<TutorCodeForm mode="create" />);
...
test("edit: tutor URL is read-only, fields are pre-filled, and the shareable link shows", async () => {
  const shareUrl = "http://localhost:3000/a1b2c3d4e5";
  const screen = await render(
    <TutorCodeForm
      mode="edit"
      code="a1b2c3d4e5"
      initialTutorUrl={TUTOR_URL}
  ...
  const url = screen.getByLabelText(/Tutor YAML URL/);
  expect((url.element() as HTMLInputElement).readOnly).toBe(true);
  expect(inputValue(url)).toBe(TUTOR_URL);
  ...
  const link = screen.getByLabelText("Tutor Code link", { exact: true });
...
test("edit: renders the server action's error message", async () => {
  updateResult = { status: "error", message: "This tutor code no longer exists. Reload the list." };
  const screen = await render(
    <TutorCodeForm
      mode="edit"
      code="a1b2c3d4e5"
