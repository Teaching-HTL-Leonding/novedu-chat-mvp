...
import { TutorCodeError } from "@/app/tutor-code-error";

// The student-facing explanation for every way a tutor code can be rejected.
...
test("unknown code: explains the code does not exist, no <time>", async () => {
  const screen = await render(
    <TutorCodeError verification={{ ok: false, reason: "unknown-code" }} />,
  );
  await expect.element(screen.getByRole("heading", { name: "Unknown tutor code" })).toBeVisible();
...
test("not-started: names when the code becomes active", async () => {
  ...
  await expect
    .element(screen.getByRole("heading", { name: "Tutor not available yet" }))
    .toBeVisible();
...
test("expired: names when the code was valid until", async () => {
  ...
  await expect.element(screen.getByRole("heading", { name: "Tutor code expired" })).toBeVisible();
...
test("lookup-failed: framed as transient, no <time>", async () => {
  const screen = await render(
    <TutorCodeError verification={{ ok: false, reason: "lookup-failed" }} />,
  );
  await expect
    .element(screen.getByRole("heading", { name: "Tutor codes temporarily unavailable" }))
    .toBeVisible();
  expect(document.querySelector("time")).toBeNull();
});
