...
- `src/pages/llms.txt.ts`, `llms-full.txt.ts`, `[...slug].md.ts` + `src/lib/llms.ts`
  ...
  content collection the pages come from (so it follows corpus regenerations with
  no extra step):
  ...
  - `/docs/<section>/<chapter>.md` — each chapter's **Markdown twin**: appending
    ...
    (banner comment stripped) plus its title, description and a footer linking back
    to the index and the HTML page.
